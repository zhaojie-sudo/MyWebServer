...
// 线程池类，将它定义为模板类是为了代码复用，模板参数Task是任务类
...
template< typename Task >
threadpool< Task >::threadpool(int thread_number, int max_requests) : 
...
void threadpool< Task >::run() {
    ...
    while (!m_stop) {
        // 申请资源 资源-1 
        m_queuestat.wait(); /* 这里如果可用资源为0 则阻塞直至有可用资源 */
        m_queuelocker.lock();
